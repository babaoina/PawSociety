...
class MyFirebaseMessagingService : FirebaseMessagingService() {
    ...
    private fun sendNotification(
        ...
        // For message type, show only title without message body content (silent notification)
        ...
        // Update badge on notification arrival for message type
        if (type == "message") {
            coroutineScope.launch {
                try {
                    val sessionManager = SessionManager(applicationContext)
                    val currentUser = sessionManager.getCurrentUser()
                    if (currentUser != null) {
                        val chatRepository = com.example.pawsociety.data.repository.ChatRepository()
                        ...
                        if (result.isSuccess) {
                            ...
                            com.example.pawsociety.util.InboxBadgeManager.updateBadgeManually(
                                totalUnread
                            )
                        }
                    }
                } catch (e: Exception) {
                    Log.d(tag, "Failed to update badge: ${e.message}")
                }
            }
        }
    }
